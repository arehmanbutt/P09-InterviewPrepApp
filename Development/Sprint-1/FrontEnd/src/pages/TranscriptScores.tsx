...
export default function TranscriptScores({ interviewId }: Props) {
  ...
  if (!interviewId)
    return (
      <div className="p-3 text-sm text-red-400">No interview selected</div>
    );

  if (loading)
    return <div className="p-3 text-sm text-gray-400">Loading scores…</div>;

  if (error)
    return <div className="p-3 text-sm text-red-400">Error: {error}</div>;
  ...
  return (
    <div className="p-4 bg-white/5 rounded-md">
      ...
      <div className="mt-2 text-sm">
        Overall score: <strong>{overall ?? "N/A"}</strong>
      </div>

      <div className="mt-4 space-y-3">
        {items.length === 0 && (
          <div className="text-sm text-gray-400">No per-question data</div>
        )}
