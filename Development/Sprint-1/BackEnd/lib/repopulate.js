...
const MONGO =
  process.env.MONGO_URL || process.env.MONGO_URI || process.env.MONGO;
...
try {
  ...
  // 1) Optional: manually backup first

  // 2) Remove existing docs
  const del = await Question.deleteMany({});
  console.log(
    `Deleted ${del.deletedCount} documents from Question collection.`
  );
  ...
  if (!Array.isArray(items)) {
    throw new TypeError("paraphrased_qas.json is not an array");
  }
